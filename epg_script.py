...
def run(target_region=None):
    ...
    for region_name, nid in items:
        ...
        session.cookies.update({'fv_location': nid, 'userNid': nid})
        ...
        try:
            r_chan = session.get(f"https://www.freeview.co.uk/api/channel-list?nid={nid}", timeout=15)
        ...
        for day in range(DAYS):
            ...
            try:
                r = session.get(f"https://www.freeview.co.uk/api/tv-guide?nid={nid}&start={ts}", timeout=15)
                if r.status_code != 200: 
                ...
                for chan in day_chans:
                    ...
                    for ev in chan.get('events', []):
                        ...
                        try:
                            ...
                            e_time = (start_dt + timedelta(hours=h, minutes=m)).strftime('%Y%m%d%H%M%S +0000')
        ...
        if total_to_fetch > 0:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                futures = [executor.submit(fetch_deep_info, c, u, session) for c, u in total_missing_list]
